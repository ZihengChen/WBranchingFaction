...
class SelectionCounter:
    ...
    def countSlt_ByTauDecay(self,measuredf, withWeights=True, withSF=False, withWeightsAdjust=None):
        yields = []
        ...
        if withWeights is True:
            for i in range(1,22,1):
                ...
                if withSF:
                    if withWeightsAdjust is None:
                        w = np.sum(temp.eventWeight)
                    else:
                        w = np.sum(temp.eventWeight * withWeightsAdjust)
                else:
                    if withWeightsAdjust is None:
                        w = np.sum(temp.eventWeight/temp.eventWeightSF)
                    else:
                        w = np.sum(temp.eventWeight/temp.eventWeightSF * withWeightsAdjust)
